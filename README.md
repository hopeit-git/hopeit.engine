...
- **JUN 2021**: `hopeit.engine` version 0.4.0 introduced app-visualizer plugin, that allows to view in an web browser the current running configuration and dependencies between apps and events of a deployments. Also enhaced stream processing by adding support for multiple queues for the same stream. In version 0.5.0 we started adding more DevOps tools: the first one is `log-streamer` plugin, that allows to parse app logs and send data to a stream. This will be used to monitor applications and dump logs to persistent storage.
