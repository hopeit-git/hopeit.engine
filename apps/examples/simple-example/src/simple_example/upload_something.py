...
async def __preprocess__(
    payload: None, context: EventContext, request: PreprocessHook, *, something_id: str
) -> Union[str, FileUploadInfo]:
    uploaded_files = []
    save_path = Path(str(context.env["upload_something"]["save_path"]))
    ...
    async for file_hook in request.files():
        ...
        uploaded_file = UploadedFile(
            file_hook.name, file_name, save_path.as_posix(), size=file_hook.size
        )
...
async def create_items(
    payload: FileUploadInfo, context: EventContext, *, something_id: str
) -> List[Something]:
    """
    ...
    for item in payload.uploaded_files:
        logger.info(
            context,
            "Creating something from uploaded item...",
            extra=extra(
                file_id=item.file_id, user=payload.user, something_id=something_id, size=item.size
            ),
        )
        result.append(Something(id=item.file_id, user=payload.object.user))
    return result
