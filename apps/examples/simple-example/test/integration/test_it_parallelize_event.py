...
async def test_parallelize_event(
    ...
    assert (
        msg
        == f"events submitted to stream: {app_config.events['shuffle.parallelize_event'].write_stream.name}"
    )
    assert (
        response.headers.get("X-Stream-Name")
        == app_config.events["shuffle.parallelize_event"].write_stream.name
    )
