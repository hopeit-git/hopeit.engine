...
def sample_file_id():
    ...
    with open(
        f"/tmp/hopeit/simple_example.{APP_VERSION}.fs_storage.path/2020/05/01/00/{test_id1}.json",
        "w",
    ) as f:
        f.write(json_str1)
        f.flush()
    with open(
        f"/tmp/hopeit/simple_example.{APP_VERSION}.fs_storage.path/2020/05/01/00/{test_id2}.json",
        "w",
    ) as f:
        f.write(json_str2)
        f.flush()
