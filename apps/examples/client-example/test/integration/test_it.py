...
async def custom_app_call(
    ...
    **kwargs,
) -> Union[str, MockAuthInfo, SomethingNotFound, Something]:
    ...
    if (
        ...
        and event == "query_something"
        and "item_id" in kwargs
    ):
    ...
    if app_connection == "simple_example_conn" and event == "query_something":
        raise UnhandledResponse(
            "Missing 400 status handler, use `responses` to handle this exception", "", 400
        )
    raise NotImplementedError("Test case not implemented in mock_app_call")


async def custom_app_call_list(
    app_connection: str,
    *,
    event: str,
...
async def test_handle_responses_something(monkeypatch, client_app_config):  # noqa: F811
    ...
    assert (
        result == "Got 200 response with: 'Something(id='id_something', user=User(id='test',"
...
async def test_call_unsecure(monkeypatch, client_app_config):  # noqa: F811
    ...
    context = create_test_context(client_app_config, "call_unsecured")
