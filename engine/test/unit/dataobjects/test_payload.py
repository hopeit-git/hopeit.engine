...
def test_to_obj_list_mixed():
    ...
    assert Payload.to_obj(
        ...
        {"data": {"id": "1", "nested": {"ts": ts}, "value": "ok-1"}, "item": 1},
        {"data": {"id": "2", "nested": {"ts": ts}, "value": "ok-2"}, "item": 2},
    ]
...
def test_to_obj_dict_mixed():
    ...
    assert Payload.to_obj(
        ...
        "item1": {
            "data": {"id": "1", "nested": {"ts": ts}, "value": "ok-1"},
            "item": 1,
        },
        "item2": {
            "data": {"id": "2", "nested": {"ts": ts}, "value": "ok-2"},
            "item": 2,
        },
    }
...
def test_to_obj_dataobject():
    ts = datetime.fromtimestamp(0, tz=timezone.utc)
    assert Payload.to_obj(MockData(id="test", value="ok", nested=MockNested(ts=ts))) == {
        "id": "test",
        "value": "ok",
        "nested": {"ts": ts},
    }
...
def test_from_json_dataobject():
    assert Payload.from_json(
        ...
        id="test",
        value="ok",
        nested=MockNested(ts=datetime.fromtimestamp(0.0).astimezone(timezone.utc)),
    )
...
def test_from_obj_dataobject_dict():
    ...
    assert Payload.from_obj(
        ...
        "item1": MockData(id="test1", value="ok", nested=MockNested(ts=ts)),
        "item2": MockData(id="test2", value="ok", nested=MockNested(ts=ts)),
    }
...
def test_from_obj_dataobject_list():
    ...
    assert Payload.from_obj(
        ...
        MockData(id="test1", value="ok", nested=MockNested(ts=ts)),
        MockData(id="test2", value="ok", nested=MockNested(ts=ts)),
    ]


def test_from_obj_list():
    assert Payload.from_obj([{"value": "ok1"}, {"value": "ok2"}], List[Dict[str, str]]) == [
        {"value": "ok1"},
        {"value": "ok2"},
    ]
...
def test_from_obj_dict():
    assert Payload.from_obj(
        {"item1": {"value": "ok1"}, "item2": {"value": "ok2"}},
...
def test_from_json_dataobject_do_not_validate_null():
    data = '{"id": "test", "value": null}'
    assert Payload.from_json(data, MockDataNullable) == MockDataNullable(id="test", value=None)  # type: ignore
...
def test_from_obj_dataobject_do_not_validate_none():
    ...
    assert Payload.from_obj(data, MockDataNullable) == MockDataNullable(id="test", value=None)  # type: ignore
...
def test_from_json_invalid_types():
    with pytest.raises(ValidationError):
        Payload.from_json(
            '{"id": "1", "value": "ok-1", "nested": {"ts": "INVALID DATE"}}', MockData
        )
    ...
    with pytest.raises(ValidationError):
        Payload.to_json(
            MockData(
                id=42,
                value="ok-value",
                nested=MockNested(ts=datetime.now(tz=timezone.utc)),
            )
        )  # type: ignore
...
def test_to_obj_kwargs():
    original = MockWithDecimal(number=Decimal("1.2"))
    pythonic = Payload.to_obj(original)
    json = Payload.to_obj(original, mode="json")

    pythonic == {"number": Decimal("1.2")}
    json == {"number": "1.2"}

    assert Payload.from_obj(pythonic, MockWithDecimal) == original
    assert Payload.from_obj(json, MockWithDecimal) == original


def test_to_json_kwargs():
    ts = datetime.fromtimestamp(0, tz=timezone.utc)
    original = MockData(id="1", value="ok-1", nested=MockNested(ts=ts))
    exclude = Payload.to_json(original, exclude="nested")
    assert exclude == '{"id":"1","value":"ok-1"}'
