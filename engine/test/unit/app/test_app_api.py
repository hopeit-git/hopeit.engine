...
from mock_app import (
    ...
    MockData,
    mock_app_api_post,
    mock_app_api_get_list,
)
from mock_app import mock_api_app_config, mock_api_spec


def test_api_from_config(monkeypatch, mock_api_spec, mock_api_app_config):
    monkeypatch.setattr(api, "spec", mock_api_spec)
    spec = api.api_from_config(
        mock_app_api_get,
        app_config=mock_api_app_config,
...
def test_api_from_config_missing(monkeypatch, mock_api_spec, mock_api_app_config):
    monkeypatch.setattr(api, "spec", mock_api_spec)
...
def test_event_api(monkeypatch, mock_api_spec, mock_api_app_config):
    monkeypatch.setattr(api, "spec", mock_api_spec)
    spec = api.event_api(
...
def test_event_api_post(monkeypatch, mock_api_spec, mock_api_app_config):
    monkeypatch.setattr(api, "spec", mock_api_spec)
...
def test_app_base_route_name(mock_api_app_config):
    assert api.app_base_route_name(mock_api_app_config.app) == "/api/mock-app-api/test"
    assert (
        api.app_base_route_name(mock_api_app_config.app, plugin=mock_api_app_config.app)
