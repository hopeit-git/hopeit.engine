...
def test_find_event_handler(mock_app_config):  # noqa: F811
    impl = find_event_handler(
        app_config=mock_app_config, event_name='mock_event', event_info=mock_app_config.events['mock_event']
    )
...
def test_find_event_handler_not_found(mock_app_config):  # noqa: F811
    with pytest.raises(ImportError):
        find_event_handler(
            app_config=mock_app_config, event_name='unknown', event_info=mock_app_config.events['mock_event']
        )


def test_find_event_handler_custom_impl(mock_app_config):  # noqa: F811
    impl = find_event_handler(
