...
comp = {
    ...
    Compression.BZ2:
        b'BZh91AY&SYA&\xba\x95\x00\x00\x01\x84\x01\xa0\x00\x00\x80\x00\x08 \x00 '
        b'\xaamA\x98\xba\x83\xc5\xdc\x91N\x14$\x10I\xae\xa5@',
    Compression.BZ2_MIN:
        b'BZh11AY&SYA&\xba\x95\x00\x00\x01\x84\x01\xa0\x00\x00\x80\x00\x08 \x00 '
        b'\xaamA\x98\xba\x83\xc5\xdc\x91N\x14$\x10I\xae\xa5@',
    Compression.BZ2_MAX:
        b'BZh91AY&SYA&\xba\x95\x00\x00\x01\x84\x01\xa0\x00\x00\x80\x00\x08 \x00 '
        b'\xaamA\x98\xba\x83\xc5\xdc\x91N\x14$\x10I\xae\xa5@',
    Compression.GZIP:
        b'\x1f\x8b\x08\x00\xbb\x9d\xe4^\x02\xffst\x1c\x05\xa3`\x14\x0cw\x00'
        b'\x00\x01.\xa0Q\xe8\x03\x00\x00',
    Compression.GZIP_MIN:
        b'\x1f\x8b\x08\x00\xad\x9e\xe4^\x04\xffst\x1c\x05\xa3!0\x1a\x02\xc3'
        b'=\x04\x00\x01.\xa0Q\xe8\x03\x00\x00',
    Compression.GZIP_MAX:
        b'\x1f\x8b\x08\x00\xeb\x9e\xe4^\x02\xffst\x1c\x05\xa3`\x14\x0cw\x00'
        b'\x00\x01.\xa0Q\xe8\x03\x00\x00',
    Compression.LZ4:
        b'\x04"M\x18h@\xe8\x03\x00\x00\x00\x00\x00\x00\xf1\x0e\x00\x00\x00\x1f'
...
def test_compress():
    ...
    assert compress(data, Compression.ZIP_MIN) == comp[Compression.ZIP_MIN] == zlib.compress(data, 1)
    assert compress(data, Compression.ZIP_MAX) == comp[Compression.ZIP_MAX] == zlib.compress(data, 9)
    ...
    assert compress(data, Compression.GZIP)[10:] == comp[Compression.GZIP][10:] == gzip.compress(data, 9)[10:]
    assert compress(data, Compression.GZIP_MIN)[10:] == comp[Compression.GZIP_MIN][10:] == gzip.compress(data, 1)[10:]
    assert compress(data, Compression.GZIP_MAX)[10:] == comp[Compression.GZIP_MAX][10:] == gzip.compress(data, 9)[10:]
    assert compress(data, Compression.LZ4) == comp[Compression.LZ4] == lz4.frame.compress(data, 3)
    assert compress(data, Compression.LZ4_MIN) == comp[Compression.LZ4_MIN] == lz4.frame.compress(data, 0)
    assert compress(data, Compression.LZ4_MAX) == comp[Compression.LZ4_MAX] == lz4.frame.compress(data, 16)
