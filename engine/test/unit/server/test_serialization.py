...
async def test_deserialize():
    ...
    assert (
        await deserialize(ser[Serialization.PICKLE3], Serialization.PICKLE3, Compression.NONE, Data)
        == data
    )
    assert (
        await deserialize(ser[Serialization.PICKLE4], Serialization.PICKLE4, Compression.NONE, Data)
        == data
    )
    if pickle5_available:
        assert (
...
async def test_serialize_primitives():
    ...
    assert (
        await deserialize(b'{"value": "test"}', Serialization.JSON_UTF8, Compression.NONE, str)
        == "test"
    )
    ...
    assert (
        await deserialize(b'{"value":42.5}', Serialization.JSON_UTF8, Compression.NONE, float)
        == 42.5
    )

    assert await serialize(True, Serialization.JSON_UTF8, Compression.NONE) == b'{"value":true}'
    assert (
        await deserialize(b'{"value":true}', Serialization.JSON_UTF8, Compression.NONE, bool)
        is True
    )
