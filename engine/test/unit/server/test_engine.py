...
async def create_engine(
    app_config: AppConfig, plugin: AppConfig, enabled_groups: Optional[List[str]] = None
) -> AppEngine:
    if enabled_groups is None:
        enabled_groups = []
    engine = await AppEngine(
        app_config=app_config, plugins=[plugin], enabled_groups=enabled_groups
    ).start()
    assert engine.stream_manager is not None
    return engine


async def create_plugin_engine(
    plugin: AppConfig, enabled_groups: Optional[List[str]] = None
) -> AppEngine:
    if enabled_groups is None:
        enabled_groups = []
...
async def invoke_execute(
    ...
    res = await engine.execute(context=context, query_args=query_args, payload=payload)
    assert res == expected
...
async def test_read_stream(monkeypatch, mock_app_config, mock_plugin_config):
    ...
    monkeypatch.setattr(MockEventHandler, "input_payload", payload)
    monkeypatch.setattr(
        MockEventHandler, "call_function", mock_event_dataobject_payload.entry_point
    )
...
async def test_read_write_stream_propagate_queue(
    ...
    mock_app_config.events[
        "mock_read_write_stream"
    ].write_stream.queue_strategy = StreamQueueStrategy.PROPAGATE
    engine = await create_engine(app_config=mock_app_config, plugin=mock_plugin_config)
    monkeypatch.setattr(engine, "stream_manager", MockStreamManager(address="test"))
    res = await engine.read_stream(event_name="mock_read_write_stream", test_mode=True)
    assert res == expected
...
async def test_read_write_stream_drop_queue(
    ...
    mock_app_config.events[
        "mock_read_write_stream"
    ].write_stream.queue_strategy = StreamQueueStrategy.DROP
    engine = await create_engine(app_config=mock_app_config, plugin=mock_plugin_config)
    monkeypatch.setattr(engine, "stream_manager", MockStreamManager(address="test"))
    res = await engine.read_stream(event_name="mock_read_write_stream", test_mode=True)
    assert res == expected
...
async def test_read_write_stream_new_queue(
    ...
    monkeypatch.setattr(MockStreamManager, "test_payload", payload)
    mock_app_config.events["mock_read_write_stream"].write_stream.queues = ["custom"]
    engine = await create_engine(app_config=mock_app_config, plugin=mock_plugin_config)
    monkeypatch.setattr(engine, "stream_manager", MockStreamManager(address="test"))
    res = await engine.read_stream(event_name="mock_read_write_stream", test_mode=True)
    assert res == expected
...
async def test_read_write_stream_multiple_queues(
    ...
    monkeypatch.setattr(MockStreamManager, "test_payload", payload)
    monkeypatch.setattr(MockStreamManager, "test_queue", "original")
    mock_app_config.events["mock_read_write_stream"].read_stream.queues = ["q1", "q2"]
    mock_app_config.events["mock_read_write_stream"].write_stream.queues = ["q3", "q4"]
    engine = await create_engine(app_config=mock_app_config, plugin=mock_plugin_config)
    monkeypatch.setattr(engine, "stream_manager", MockStreamManager(address="test"))
    res = await engine.read_stream(event_name="mock_read_write_stream", test_mode=True)
    assert res == expected
    ...
    assert MockStreamManager.last_read_stream_names[-2:] == [
        "mock_read_write_stream.read.q1",
        "mock_read_write_stream.read.q2",
    ]
    assert MockStreamManager.last_read_queue_names[-2:] == ["original", "original"]
    assert MockStreamManager.last_write_stream_names[-4:] == [
        "mock_read_write_stream.write.q3",
        "mock_read_write_stream.write.q4",
        "mock_read_write_stream.write.q3",
        "mock_read_write_stream.write.q4",
    ]
    assert MockStreamManager.last_write_queue_names[-4:] == ["q3", "q4", "q3", "q4"]
    await engine.stop()
...
async def test_read_write_stream_multiple_queues_propagate(
    ...
    assert MockStreamManager.last_read_queue_names[-2:] == ["q1", "q2"]
    assert MockStreamManager.last_write_stream_names[-4:] == [
        "mock_read_write_stream.write.q3",
        "mock_read_write_stream.write.q4",
        "mock_read_write_stream.write.q3",
    ...
    assert MockStreamManager.last_write_queue_names[-4:] == ["q1", "q1", "q2", "q2"]
    await engine.stop()
...
async def test_read_write_stream_multiple_queues_propagate_AUTO(
    ...
    monkeypatch.setattr(MockStreamManager, "test_payload", payload)
    monkeypatch.setattr(MockStreamManager, "test_queue", "custom")
    mock_app_config.events["mock_read_write_stream"].read_stream.queues = ["q1", "AUTO"]
    ...
    assert MockStreamManager.last_read_stream_names[-2:] == [
        "mock_read_write_stream.read.q1",
        "mock_read_write_stream.read",
    ]
    assert MockStreamManager.last_read_queue_names[-2:] == ["custom", "custom"]
    assert MockStreamManager.last_write_stream_names[-4:] == [
        "mock_read_write_stream.write.q3",
        "mock_read_write_stream.write.custom",
        "mock_read_write_stream.write.q3",
        "mock_read_write_stream.write.custom",
    ]
...
async def test_read_stream_timeout_fail(monkeypatch, mock_app_config, mock_plugin_config):
    payload = MockData("timeout")
...
async def test_read_stream_stop_and_recover(monkeypatch, mock_app_config, mock_plugin_config):
    ...
    monkeypatch.setattr(
        MockStreamManager,
        "error_pattern",
        [None, TypeError(), None, StreamOSError(), None],
    )
    engine = await create_engine(app_config=mock_app_config, plugin=mock_plugin_config)
...
async def test_start_all_groups(monkeypatch, mock_app_config, mock_plugin_config):
    ...
    engine = await create_engine(
        app_config=mock_app_config, plugin=mock_plugin_config, enabled_groups=None
    )
    assert all(
        event_name in engine.effective_events for event_name in mock_app_config.events.keys()
    )
