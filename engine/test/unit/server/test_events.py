...
from mock_app import MockData, MockResult, mock_app_config  # type: ignore
...
async def mock_handle_request_response_event(app_config, *, payload, expected):
    ...
    async for response in handler.handle_async_event(
        context=context, query_args={"query_arg1": payload.value}, payload=payload
    ):
...
async def mock_handle_postprocess(app_config, *, payload, expected, expected_response):
    ...
    response = await handler.postprocess(
        context=context, payload=intermediate_result, response=hook
    )
...
async def mock_handle_spawn_event(app_config, *, payload, expected, stream_name):
    ...
    event_count = 0
    async for result in handler.handle_async_event(
        context=context, query_args={}, payload=payload.value
    ):
...
async def test_handle_spawn_event(monkeypatch, mock_app_config):
    ...
    await mock_handle_spawn_event(
        ...
        stream_name="mock_write_stream_event",
    )
...
def test_get_event_settings(mock_app_config):
    ...
    assert settings.logging == EventLoggingConfig(
        extra_fields=["value"], stream_fields=["stream.msg_id"]
    )
    assert settings.stream == EventStreamConfig(
        ...
        serialization=Serialization.JSON_BASE64,
    )
    assert settings(datatype=CustomEventSettings) == CustomEventSettings(
        custom_setting=CustomSetting(custom="value")
    )
    assert settings(
        key="custom_extra_settings", datatype=CustomEventSettings
    ) == CustomEventSettings(custom_setting=CustomSetting(custom="value"))


def test_get_event_settings_split_event(mock_app_config):
