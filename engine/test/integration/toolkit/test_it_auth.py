...
from mock_app import mock_app_config, mock_client_app_config
...
def test_token_lifecycle(mock_app_config):
...
def test_client_tokens(mock_app_config, mock_client_app_config):
    ...
    client_context = _setup_client_context(
        mock_client_app_config, mock_app_config, register_client_key=True
    )
...
def test_client_not_registered(mock_app_config, mock_client_app_config):
    server_context = _setup_server_context(mock_app_config)
    client_context = _setup_client_context(
...
def test_auth_method_unsecured(mock_app_config):
    context = _setup_server_context(mock_app_config)
    assert auth.validate_auth_method(AuthType.UNSECURED, data="", context=context) is None
    assert context.auth_info["allowed"]
    assert context.auth_info["auth_type"] == AuthType.UNSECURED


def test_auth_method_basic(mock_app_config):
    context = _setup_server_context(mock_app_config)
    assert auth.validate_auth_method(AuthType.BASIC, data="dGVzdDpwYXNz", context=context) is None
    assert context.auth_info["allowed"]
...
def test_auth_method_bearer(mock_app_config):
    ...
    token = auth.new_token(mock_app_config.app_key(), payload)
    assert auth.validate_auth_method(AuthType.BEARER, data=token, context=context) is None
    assert context.auth_info["allowed"]
    assert context.auth_info["auth_type"] == AuthType.BEARER
    assert context.auth_info["payload"] == auth.decode_token(token)


def test_auth_method_refresh(mock_app_config):
    context = _setup_server_context(mock_app_config)
    payload = {
        "test": "test_value",
        "exp": datetime.now(tz=timezone.utc) + timedelta(seconds=2),
    ...
    token = auth.new_token(mock_app_config.app_key(), payload)
    assert auth.validate_auth_method(AuthType.REFRESH, data=token, context=context) is None
    assert context.auth_info["allowed"]
...
def _setup_client_context(
    app_config: AppConfig,
    server_app_config: AppConfig,
    register_client_key: bool = True,
) -> EventContext:
    _init_engine_logger(app_config)
    assert app_config.server
    ...
    if register_client_key:
        os.rename(
            pathlib.Path(app_config.server.auth.secrets_location)
            / "public"
            / f"{app_config.app_key()}_pub.pem",
            pathlib.Path(server_app_config.server.auth.secrets_location)
            / "public"
            / f"{app_config.app_key()}_pub.pem",
        )
    return EventContext(
...
def _init_engine_logger(mock_app_config):
