...
from mock_app import mock_event_logging as mock_event, mock_app_config  # type: ignore
...
def _event_context(mock_app_config):
    return EventContext(
...
def _get_app_logger(monkeypatch, mock_app_config):
...
def _get_app_extra_logger(monkeypatch, mock_app_config):
    _patch_logger(monkeypatch)
    event_settings = EventSettings(logging=EventLoggingConfig(extra_fields=["field1", "field2"]))
...
def _get_engine_logger(monkeypatch, mock_app_config):
    _patch_logger(monkeypatch)
...
def _get_engine_extra_logger(monkeypatch, mock_app_config):
    _patch_logger(monkeypatch)
...
def _get_cli_logger(monkeypatch):
...
def test_app_logger_traceback(monkeypatch, mock_app_config):
    logger = _get_app_logger(monkeypatch, mock_app_config)
...
def test_get_app_logger_extra(monkeypatch, mock_app_config):
...
def test_engine_logger(monkeypatch, mock_app_config):
    ...
    logger.start(context, extra=server_logging.extra_values([], field1="value1", field2=42))
    assert (
        MockHandler.formatter.format(MockHandler.record)[24:]
    ...
    logger.done(
        context,
        extra=server_logging.combined(server_logging.extra_values([], field1="value1", field2=42)),
    )
    assert (
        MockHandler.formatter.format(MockHandler.record)[24:]
...
def test_engine_logger_no_context(monkeypatch, mock_app_config):
    logger = _get_engine_logger(monkeypatch, mock_app_config)
    ...
    assert (
        MockHandler.formatter.format(MockHandler.record)[24:] == "| WARNING "
...
def test_engine_logger_traceback(monkeypatch, mock_app_config):
    logger = _get_engine_logger(monkeypatch, mock_app_config)
...
def test_engine_extra_logger(monkeypatch, mock_app_config):
    logger, extra = _get_engine_extra_logger(monkeypatch, mock_app_config)
    context = _event_context(mock_app_config)

    logger.info(context, "Log message", extra=extra(field1="value1", field2=42))
    assert (
        MockHandler.formatter.format(MockHandler.record)[24:]
        == "| INFO | mock_app test mock_event_logging test_host test_pid | Log message "
        "| extra.field1=value1 | extra.field2=42 "
    ...
    logger.warning(context, "Log message", extra=extra(field1="value1", field2=42))
    assert (
        MockHandler.formatter.format(MockHandler.record)[24:]
        == "| WARNING | mock_app test mock_event_logging test_host test_pid | Log message "
        "| extra.field1=value1 | extra.field2=42 "
    ...
    logger.error(context, "Log message", extra=extra(field1="value1", field2=42))
    assert (
        MockHandler.formatter.format(MockHandler.record)[24:]
        == "| ERROR | mock_app test mock_event_logging test_host test_pid | Log message "
        "| extra.field1=value1 | extra.field2=42 "
    ...
    logger.debug(context, "Log message", extra=extra(field1="value1", field2=42))
    assert (
        MockHandler.formatter.format(MockHandler.record)[24:]
    ...
    logger.done(context, extra=server_logging.combined(extra(field1="value1", field2=42)))
    assert (
        MockHandler.formatter.format(MockHandler.record)[24:]
        == "| INFO | mock_app test mock_event_logging test_host test_pid | DONE "
        "| extra.field1=value1 | extra.field2=42 "
    ...
    logger.ignored(context, extra=server_logging.combined(extra(field1="value1", field2=42)))
    assert (
        MockHandler.formatter.format(MockHandler.record)[24:]
        == "| WARNING | mock_app test mock_event_logging test_host test_pid | IGNORED "
        "| extra.field1=value1 | extra.field2=42 "
...
def test_cli_logger(monkeypatch):
