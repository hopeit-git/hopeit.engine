...
class Json(Generic[EventPayloadType]):
    ...
    def to_obj(payload: EventPayloadType, key: Optional[str] = 'value') -> Union[dict, list]:
        ...
        if isinstance(payload, _UNORDERED_LIST_TYPES):
            return [Json.to_obj(v, key=None) for v in sorted(payload)]
        if isinstance(payload, _LIST_TYPES):
            return [Json.to_obj(v, key=None) for v in payload]
        if isinstance(payload, _MAPPING_TYPES):
            return {k: Json.to_obj(v, key=None) for k, v in payload.items()}
