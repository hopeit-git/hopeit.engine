...
def init(app_key: str, app_auth_config: AuthConfig):
    ...
    if auth_config.enabled:
        ...
        try:
            ...
            logger.info(
                __name__,
                f"Found keys in {auth_config.secrets_location}",
            ...
            if auth_config.create_keys:
                logger.info(
                    __name__,
                    "Keys not found. Creating new key pair...",
                    extra=extra(app_key=app_key),
                )
        ...
        logger.warning(
            __name__,
            "Auth module not configured in server config. Auth disabled.",
            extra=extra(app_key=app_key),
        )
...
def app_public_key(app_key: str) -> RSAPublicKey:
    ...
    def _load_app_public_key():
        ...
        try:
            ...
            public_key_path = (
                pathlib.Path(auth_config.secrets_location) / "public" / f"{app_key}_pub.pem"
            )
            with open(public_key_path, "rb") as f:
                public_keys[app_key] = load_pem_public_key(f.read(), default_backend())
