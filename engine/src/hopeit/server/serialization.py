...
async def serialize(data: EventPayload, serialization: Serialization, compression: Compression) -> bytes:
    algos = _SERDESER[serialization]
    encoded = await algos[0](data, level=algos[1])
    return compress(encoded, compression)


async def deserialize(data: bytes, serialization: Serialization, compression: Compression,
