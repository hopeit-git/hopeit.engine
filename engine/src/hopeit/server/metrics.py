...
__all__ = ["metrics", "stream_metrics", "StreamStats"]
...
def metrics(context: EventContext):
    ...
    """
    return {"extra": format_extra_values(_calc_event_metrics(context), prefix="metrics.")}
...
def stream_metrics(context: EventContext):
    ...
    :return: dictionary than can be passed to logging extra= parameter
    """
    return {"extra": format_extra_values(_calc_stream_metrics(context), prefix="metrics.")}


def _calc_event_metrics(context: EventContext):
    duration = 1000.0 * (datetime.now(tz=timezone.utc) - context.creation_ts).total_seconds()
    return {"duration": f"{duration:.3f}"}
...
def _calc_stream_metrics(context: EventContext):
    ...
    if request_ts:
        ...
        elapsed = 1000.0 * (datetime.now(tz=timezone.utc) - request_dt).total_seconds()
...
class StreamStats:
    ...
    def calc(self) -> Dict[str, Union[int, float]]:
        ...
        avg_rate_success = (
            (1000.0 * (self.total_event_count - self.total_error_count) / total_elapsed)
            if total_elapsed
            else 0.0
        )
        avg_duration = (
            (1.0 * total_elapsed / self.total_event_count) if self.total_event_count else 0.0
        )
        ...
        rate_success = (
            (1000.0 * (self.event_count - self.error_count) / partial_elapsed)
            if partial_elapsed
            else 0.0
        )
        duration = (1.0 * partial_elapsed / self.event_count) if self.event_count else 0.0
        error_rate = (
            (1.0 * self.total_error_count / self.total_event_count)
            if self.total_event_count
            else 0.0
        )
        success = 1.0 - error_rate
        partial_error_rate = (
            (1.0 * self.error_count / self.event_count) if self.event_count else 0.0
        )
