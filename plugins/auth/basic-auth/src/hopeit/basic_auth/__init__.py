...
__all__ = ["ContextUserInfo", "AuthInfoExtended", "AuthInfo", "authorize", "set_refresh_token"]
...
class AuthInfoExtended:
    ...
    def to_auth_info(self):
        return AuthInfo(
            access_token=self.access_token, token_type=self.token_type, renew=self.renew
        )
...
def authorize(context: EventContext, user_info: ContextUserInfo, now: datetime) -> AuthInfoExtended:
    """
    ...
    renew_in = int(
        ...
        * max(
            ...
            - 1.0 * cfg.access_token_renew_window * (1.0 + 0.5 * random.random()),
            0.5 * cfg.access_token_expiration * (0.5 * random.random() + 0.5),
        )
    )
    token = _new_access_token(
        Payload.to_obj(user_info),  # type: ignore[arg-type]
        context,
        now,
        cfg.access_token_expiration,
        renew_in,
    )
...
def set_refresh_token(
    app: AppDescriptor, auth_info: dict, payload: AuthInfoExtended, response: PostprocessHook
):
    """
...
def _new_access_token(info: dict, context: EventContext, now: datetime, timeout: int, renew: int):
    ...
    """
    auth_payload = {**info, "iat": now, "exp": now + timedelta(seconds=timeout), "renew": renew}
    return auth.new_token(context.app_key, auth_payload)


def _new_refresh_token(info: dict, context: EventContext, now: datetime, timeout: int):
    """
    Returns a new refresh token encoding `info` and expiring in `refresh_token_expiration` seconds
    """
    auth_payload = {**info, "iat": now, "exp": now + timedelta(seconds=timeout)}
    return auth.new_token(context.app_key, auth_payload)
